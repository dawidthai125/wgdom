...
const afterReload = await page.evaluate((id) => ({
  inLs: JSON.parse(localStorage.getItem("kw-archive") || "[]").some((w) => w.id === id),
  count: JSON.parse(localStorage.getItem("kw-archive") || "[]").length,
}), TEST_ID);
...
const weekLabels = await page.getByRole("button", { name: /03\.02\.2099/ }).count();
...
console.log(JSON.stringify({ afterReload, visible, weekLabels, deleteOk }, null, 2));
