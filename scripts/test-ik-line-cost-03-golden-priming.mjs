/**
 * IK-LINE-COST-03 — Golden Priming Automatic Cost
 *
 * CLASS-A leaf cw.knr.nnrnkb.1134-01.m2 · Catalog First · no Owner / HTTP.
 * Authority = computePositionCost (≠ FTO alone).
 * Money = P7 2 dp (roundPositionCostPln).
 *
 * npx vite-node scripts/test-ik-line-cost-03-golden-priming.mjs
 */
import {
  computePositionCost,
...
const WORK_ID = PRIMING_NNRNKB_1134_01_WORK_ID;
const UNIT = "m2";
const QUANTITY = 10;
const QTY_FACTOR = PRIMING_NNRNKB_1134_01_QTY_FACTOR_L_PER_M2;
const MATERIAL_KEY = PRIMING_NNRNKB_1134_01_MATERIAL_KEY;
const MATERIAL_HOST = "cw.product.atlas_uni_grunt";
const PACK_ID = FIXTURE_PRIMING_NNRNKB_1134_01_PACK_ID;
const OUR_RATE = 1.04;
const MATERIAL_BASE = 4.24;
const MARGIN_PCT = 20;
const MATERIAL_SELL = 5.09;
const EXPECTED_MAT_QTY = Number((QUANTITY * QTY_FACTOR).toFixed(6)); // 2.1
const MATERIAL_RAW = EXPECTED_MAT_QTY * MATERIAL_SELL; // 10.689
const EXPECTED_LABOR = roundPln(QUANTITY * OUR_RATE); // 10.40
const EXPECTED_MATERIAL = roundPln(MATERIAL_RAW); // 10.69
const EXPECTED_TOTAL = roundPln(EXPECTED_LABOR + EXPECTED_MATERIAL); // 21.09
...
function makeLaborWork() {
  return {
    id: WORK_ID,
    // Valid TradeId (WgdomCostCategoryId) — required by normalizeCatalogWork.
    tradeId: "MALOWANIE",
    namePl:
      "Gruntowanie podłoży preparatem ATLAS UNI-GRUNT — powierzchnie poziome (sufity) — NNRNKB 1134-01",
    unit: UNIT,
    companyPricePln: 999,
    marketQuotes: {},
    marketQuoteHistory: [],
    ...
    updatedAt: T_FRESH,
    freshnessStatus: "ok",
    keywords: ["1134-01", "gruntowanie", "atlas"],
    active: true,
    favorite: false,
    usageCount: 0,
    source: "custom",
...
eq("qty_factor_const", QTY_FACTOR, 0.21);
eq("work_id_const", WORK_ID, "cw.knr.nnrnkb.1134-01.m2");
eq("material_key_const", MATERIAL_KEY, "mat.atlas_uni_grunt");
approx("material_qty_expected", EXPECTED_MAT_QTY, 2.1);
approx("material_raw_product", MATERIAL_RAW, 10.689);
approx("material_cost_engine", EXPECTED_MATERIAL, 10.69);
approx("labor_cost_engine", EXPECTED_LABOR, 10.4);
approx("position_cost_engine", EXPECTED_TOTAL, 21.09);
...
const ready = isIkReadyToBid({
  item: { id: "golden-ik-line-cost-03" },
  financeOk: false,
  p7: {
...
console.log(`
========================================
IK-LINE-COST-03 GOLDEN PRIMING
========================================
...
console.log(
  `\nIK_LINE_COST_03_GOLDEN ${fail === 0 ? "PASS" : "FAIL"} ${pass}/${pass + fail}`,
);
