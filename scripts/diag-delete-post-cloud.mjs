...
await page.evaluate(({ id }) => {
  ...
  window.dispatchEvent(new StorageEvent("storage", { key: "kw-archive" }));
}, { id: TEST_ID });

// trigger React re-read — navigate away and back
await page.locator("nav.admin-sidebar-nav button").filter({ has: page.locator("span.flex-1", { hasText: /^Pulpit$/i }) }).first().click();
...
const inLs = await page.evaluate((id) => JSON.parse(localStorage.getItem("kw-archive") || "[]").some((w) => w.id === id), TEST_ID);
const visible = await page.locator("text=Smoke Del ETAP1D").count();

let deleteOk = false;
if (visible > 0) {
  const delCard = page.locator(".bg-card.rounded-xl.border.overflow-hidden").filter({ hasText: "Smoke Del ETAP1D" }).first();
  await delCard.locator(".flex.items-center.gap-2 > button").first().click();
...
console.log(JSON.stringify({ inLs, visible, deleteOk }, null, 2));
