...
await page.evaluate(({ id }) => {
  ...
  archive.push({
    id, weekFrom: "2099-02-03", weekTo: "2099-02-08", savedAt: new Date().toISOString(),
...
const weekBtn = page.getByRole("button", { name: /03\.02\.2099.*08\.02\.2099/ }).first();
const beforeCount = await weekBtn.count();
const inLsBefore = await page.evaluate((id) => JSON.parse(localStorage.getItem("kw-archive") || "[]").some((w) => w.id === id), TEST_ID);

// trash is inside week row — scope to parent card
const card = page.locator(".bg-card.rounded-xl.border.overflow-hidden").filter({ has: weekBtn });
await card.locator("button").filter({ has: page.locator("svg") }).first().click(); // Trash2 icon button
await page.waitForTimeout(300);
const confirmVisible = await page.getByRole("button", { name: /^Usuń$/i }).isVisible();
await page.getByRole("button", { name: /^Usuń$/i }).click();
await page.waitForTimeout(1000);

const afterCount = await page.getByRole("button", { name: /03\.02\.2099.*08\.02\.2099/ }).count();
const inLsAfter = await page.evaluate((id) => JSON.parse(localStorage.getItem("kw-archive") || "[]").some((w) => w.id === id), TEST_ID);

console.log(JSON.stringify({ beforeCount, inLsBefore, confirmVisible, afterCount, inLsAfter }, null, 2));
