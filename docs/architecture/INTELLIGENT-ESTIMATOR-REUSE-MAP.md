...
> **LIVE tip:** **FETCH** `/version.json` · expected **2.66.235 / `643bd6e`** · documentary [`../AI/09_PRODUCTION_BASELINE.md`](../AI/09_PRODUCTION_BASELINE.md) · KLAMKI persist C+D [`IK-IDENTITY-KLAMKI-CLOUD-PERSISTENCE-LOSS-CLOSEOUT.md`](./IK-IDENTITY-KLAMKI-CLOUD-PERSISTENCE-LOSS-CLOSEOUT.md)
...
| **GATED_IDENTITY_PERSIST (C+D)** | `orchestra/ik-identity-persist-glue.ts` · `orchestra/ik-autonomous-identity-writeback.ts` · `multi-dwelling/store.ts` · `multi-dwelling/cloud-push-safety.ts` | Multi-dwelling gated identity batch: suspend×N attach → **one** awaited flush → **required cloud readback**; non-browser live write opt-in | `runGatedIdentityPersist` · `runGatedIdentityPersistAwaitCloud` · `isGatedIdentityPersistSuccess` · `settleGatedIdentityPersistCloud` | Orchestra · GO-AUTO writeback | `kw-multi-dwelling-package-v1` | PRODUCTION (`643bd6e0` · UI **2.66.235**) | **≠** Rate/Catalog · **≠** Safe Merge eligibility rewrite · [`CLOSEOUT`](./IK-IDENTITY-KLAMKI-CLOUD-PERSISTENCE-LOSS-CLOSEOUT.md) |
