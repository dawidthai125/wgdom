...
> **Data:** **2026-09-24** · **KLAMKI persist C+D CLOSED/PV** · prior **2026-09-21** Decision C PRODUCTION CLOSEOUT · prior M3 docs reconcile · prior 2026-09-16 RMS/ATH→KL3 + MODEL C · prior 2026-09-15 Gap Job freeze · prior 2026-09-14 AUT-MAT
...
> **LIVE tip:** **FETCH** `/version.json` · expected **2.66.235 / `643bd6e`** · app tip **`643bd6e0`** · docs tip **`f756b27b`** (DOCUMENTATION-ONLY) · documentary [`../AI/09_PRODUCTION_BASELINE.md`](../AI/09_PRODUCTION_BASELINE.md) · KLAMKI persist C+D [`IK-IDENTITY-KLAMKI-CLOUD-PERSISTENCE-LOSS-CLOSEOUT.md`](./IK-IDENTITY-KLAMKI-CLOUD-PERSISTENCE-LOSS-CLOSEOUT.md) · persistence law Master **§22.3**
...
| **OWNER_RC1_VERIFY_CONNECT** | `owner-rc1-verify-connect-contract.ts` · `orchestra/owner-rc1-verify-connect-apply.ts` · `multi-dwelling/canonical-identity-upgrade-merge.ts` | Fail-closed Safe Merge eligibility for **exactly 2** Owner RC1 VERIFY_CONNECT leaves (+ paired mappingIds) | `applyOwnerRc1VerifyConnectIdentityToLine` · `hasOwnerRc1VerifyConnectAttestation` · `evaluateCanonicalIdentityUpgradeMerge` | Owner GO cloud identity apply | OfferBoq via gated persist + Safe Merge | PRODUCTION (`b64a5d8a` · MOPS RC1 6 lines CLOSED) | **≠** `/^p2b-/` · **≠** CLLR substitute · [`CLOSEOUT`](./IK-IDENTITY-APPLY-MOPS-ELEC-RC1-CLOSEOUT.md) |
| **GATED_IDENTITY_PERSIST (C+D)** | `orchestra/ik-identity-persist-glue.ts` · `orchestra/ik-autonomous-identity-writeback.ts` · `multi-dwelling/store.ts` · `multi-dwelling/cloud-push-safety.ts` | Multi-dwelling gated identity batch: suspend×N attach → **one** awaited flush → **required cloud readback**; non-browser live write opt-in · **do not bypass** cloud flush / durable readback / live cloud guard · law Master **§22.3** | `runGatedIdentityPersist` · `runGatedIdentityPersistAwaitCloud` · `isGatedIdentityPersistSuccess` · `settleGatedIdentityPersistCloud` | Orchestra · GO-AUTO writeback | `kw-multi-dwelling-package-v1` | PRODUCTION (app **`643bd6e0`** · UI **2.66.235** · docs tip **`f756b27b`**) | **≠** Rate/Catalog · **≠** Safe Merge eligibility rewrite · [`CLOSEOUT`](./IK-IDENTITY-KLAMKI-CLOUD-PERSISTENCE-LOSS-CLOSEOUT.md) |
| Safe Merge identity upgrade | `canonical-identity-upgrade-merge.ts` | Score-tie cloud merge: legacy → eligible leaf | `evaluateCanonicalIdentityUpgradeMerge` | `pushKeysToCloudSafe` / multi-dwelling merge | `kw-multi-dwelling-package-v1` | PRODUCTION | Eligible = `cw.knr.*` **or** RC1 CONNECT allowlist · anti-downgrade |
...
| **KL3 canonical persistence seam** | `orchestra/ik-orchestra-runtime.ts` | `executeKl3KnowledgeLookup` → `persistKl3DiscoveryEvidence` → `merge(local, ath)` → etag gate → `saveKnrDiscoveryEvidenceStore` | `persistKl3DiscoveryEvidence(store, io)` | KL3 host complete (async) | `kw-knr-discovery-evidence` | PRODUCTION (`a07ab4be` · **HISTORY tip** 2.66.230 · ≠ CURRENT LIVE **`643bd6e`** · HISTORY Decision C **`50bce20`** · HISTORY mid-chain Tender Freeze **`cc210d9`**) | REUSE canonical writer · **`merged ⊇ local`** · no new store/writer · no ATH-only filter (`MIXED` accepted) |
