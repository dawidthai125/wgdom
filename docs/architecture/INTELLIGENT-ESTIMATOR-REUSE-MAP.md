...
| **AUT-R1** | `aut-r1-accept-contract.ts` · `aut-r1-accept.ts` · (`aut-r1-from-durable-evidence.ts`) | Autonomous Labor Accept under §0.2 | `evaluateAutR1LaborAcceptContract` · tryAutR1* | `ik-labor-expert` / P5 | Work Catalog | PRODUCTION (+ Evidence wire may be WIP_LOCAL) | **≠** invent · **≠** unit auto-convert · **no** Labor Rate Engine 2 |
