...
| **Form patch Roboty (JOBS-FORM-RACE-01, v2.65.7)** | `JobsView.updateJob` — functional merge `{ ...prevJob, ...delta }` wewnątrz `setJobs`/`applyJobs`; walidacja inspektora na **merged** job; pola formularza summary przekazują **delta-only** (`{ id, pole }`) zamiast `{...selectedJob}`. **Naprawia** utratę znaków podczas szybkiego wpisywania (keystroke race). **Bez** zmian `cloud-sync` / Edge / PWRB. Test: `test-jobs-form-race-01.mjs` JF-T01–T08 |
| **Merge jobs photos (JOBS-ASSETS-SYNC-01, v2.65.9)** | `mergeJobsById` → `mergePair`: `photos[]` przez `mergePhotos` (`job-photos.ts`) — **union po `id`** (wzorzec `mergeInspectorPhotos`); identyfikator SSOT: `PhotoEntry.id` z `crypto.randomUUID()` przy `uploadPhoto`. **Naprawia** znikające zdjęcia ekipy po auto-sync (~2 s). **Bez** zmian reconcile chain / upload / Edge. Test: `test-jobs-assets-sync-01.mjs` JA-ASSETS-T01–T08 |
| **Delete photos tombstones (JOBS-PHOTOS-DELETE-SYNC-01, v2.65.10)** | `Job.deletedPhotoTombstones[]` + `mergePhotoTombstones` / `filterPhotosByTombstones` / `removePhotoWithTombstone` (`job-photos.ts`); `mergePair` przekazuje tombstones do `mergePhotos`. Parity `deletedJobFileTombstones`. **Naprawia** wracające usunięte zdjęcia po sync. Delete: admin `JobPhotoGallery` + pracownik `WorkerPhotoView`. **Bez** zmian `uploadPhoto` / reconcile chain / Edge. Test: `test-jobs-photos-delete-sync-01.mjs` JA-PHOTO-DEL-T01–T11 |
...
### 12.1.36 NG11-A3 — Discovery Fork (v2.64.0) · **PRODUCTION VERIFIED**

**Status:** **IMPLEMENTED** · **PRODUCTION VERIFIED** (2026-07-11) · flaga `pipelinePerfDiscoveryFork` default **OFF**  
...
**Status:** **RELEASED** · **PRODUCTION VERIFIED** (2026-07-11) · flaga `pipelinePerfDebouncePersist` default **OFF**  
