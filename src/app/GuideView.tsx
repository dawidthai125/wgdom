        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              {q:"Market Sync Preview (P0)", a:"Tylko Super Admin: przycisk Market Sync Preview w Bibliotece. Import CSV sklepów DIY (Leroy/Casto…) + produkty referencyjne JSON → Match (EAN/SKU/nazwa) → Preview. Zapis wyłącznie lokalny (staging). Brak Accept, Publish i zapisu cen do katalogu. Eksport/import JSON staging oraz Odśwież Match."},
              {q:"Czego tu nie ma?", a:"Market Sync P0 nie publikuje cen do Product Quotes. Pełny sync sklepów (Accept/Publish) dopiero w późniejszym etapie po osobnej decyzji."},
            ].map((item,i)=>(
              <div key={i} className="border border-border rounded-xl overflow-hidden">
                <div className="px-4 py-3 bg-secondary/30">
                  <p className="text-sm font-medium flex items-center gap-2"><HelpCircle size={13} className="text-primary shrink-0"/>{item.q}</p>
