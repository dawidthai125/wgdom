    ...
    },
    {
      ...
      content:(
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              {q:"Jak dodać pracownika do tygodnia?", a:'Kliknij "Dodaj pracownika" → zaznacz jednego lub kilku pracowników z listy (lub "Zaznacz wszystkich") → kliknij "Dodaj zaznaczonych". Jeśli tydzień jest pusty, pojawi się też przycisk "Kopiuj z poprzedniego tygodnia" — kliknij go, żeby od razu dodać tych samych co ostatnio. Dopóki chmura nie potwierdzi dodania, kolejna zmiana godzin albo zaliczki nie usuwa tej osoby z zapisu — także gdy lista w tle odświeży się ze starszym składem z chmury. Ponowne dodanie kogoś usuniętego w tym tygodniu też zostaje — stary znacznik usunięcia nie kasuje tego dodania. Świadome usunięcie nadal usuwa osobę. Dodanie nowej osoby nie zapisuje starszych, niższych godzin kogoś innego — w chmurze zostają godziny już potwierdzone.'},
              ...
              {q:"Komunikat o zapisie pipeline po zmianie godzin", a:"Zmiana godzin, dodanie pracownika albo zapis notatki / kadr zapisuje tylko tę domenę w chmurze — nie zapisuje pipeline przetargów. Komunikat „Zapis pipeline zablokowany” / reconciliation może pojawić się przy pracy z przetargami, nie przy samym zapisie godzin albo notatki. Osobny komunikat „Zapis listy płac zablokowany” to ochrona danych listy płac; konflikt zapisu to zderzenie z inną zmianą w chmurze."},
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              {q:"Rysunki techniczne (Odbiory WM)", a:"WM Druk → „Rysunki” (zaraz po Odbiory): szybkie szkice techniczne powiązane z robotą. Włączenie: Super Admin ⚙ → Moduły → Rysunki WM (lub mirror w WM → Ustawienia). Sync chmura kw-app-settings. Ściana/strzałka: przeciągnij i puść (Ghost podczas gestu; Esc anuluje; po sukcesie tool Ściana zostaje; kolejna ściana od nowego pierwszego kliknięcia — nowy gest). Narzędzia: drzwi P/W, okno, wymiar (klik ściany → Długość), wentylacja, piec, rozdzielnia, tekst, siatka/snap (siatka · kąt 45° · punkty), autosave, undo/redo, duplikacja, obrót, status Finalny↔Roboczy (toggle). Usunięcie Finalnego: najpierw oznacz jako Roboczy. Ghost nie trafia do JSON/PDF/ZIP. Desktop: Podgląd PDF · Pobierz · Drukuj. Dane: kw-wm-technical-drawings. ZIP Odbiory: „Dołącz rysunki” gdy ≥1 Final. Szkice pracownika non-final nie są na liście Rysunki."},
              {q:"Szkice Techniczne (Dokumentacja)", a:"Roboty / Inspektor / Pracownik → Dokumentacja → „Szkice Techniczne” (≠ foto Obrys, ≠ Odbiory WM → Rysunki). Flaga: Super Admin ⚙ Moduły → Szkice pracownika (OFF). Worker: własne szkice, prześlij / prześlij ponownie po „Do poprawy”. Inspektor: wszystkie szkice, edycja, „Do poprawy” (bez publikacji). Admin: Usuń · Dokumentacja · Odbiory · Dokumentacja+Odbiory (placement; bez Accept). Pulpit: sekcja Szkice (attention) znika po decyzji. Dane: kw-wm-technical-drawings (domain job_sketch · promote-copy do reception)."},
              {q:"Szkice pracownika (Dokumentacja)", a:"Panel pracownika → Dokumentacja → sekcja Szkice Techniczne (≠ foto Obrys). Włączenie: Super Admin ⚙ → Moduły → Szkice pracownika (domyślnie OFF, niezależnie od Rysunki WM). Pracownik rysuje na telefonie: przytrzymaj i przeciągnij ścianę (puść = zatwierdź); elementy: drzwi, okno, wentylacja, rozdzielnia, piec, tekst. Snap (siatka · kąt 45° · punkty). Ten sam silnik co Rysunki WM (kw-wm-technical-drawings). Usuwanie tylko szkicu roboczego przed przesłaniem."},
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              {q:"Generowanie", a:"Generuj komplet (ZIP) — paczka ze zaznaczonych szablonów i dokumentów robota. Formularz OST (pdf_form, gdy wgrany i aktywny) jest zawsze dołączany do Odbiory/ — wypełniany on-the-fly danymi adresu roboty; nie da się go odznaczyć. Struktura: folder Odbiory/ (dokumenty WM), opcjonalnie Pomiary/ (5× DOCX aktywnego raportu RAP) oraz opcjonalnie Rysunki/ (PDF rysunków Final — ten sam generator co Podgląd/Pobierz). Checkbox „Dołącz dokumenty pomiarowe” — domyślnie włączony gdy istnieje aktywny RAP produkcyjny; raporty testowe TEST-RAP są ignorowane. Checkbox „Dołącz rysunki” — domyślnie ON gdy robota ma ≥1 rysunek Final (tylko gdy moduł Rysunki włączony). Domyślnie wszystkie aktywne szablony są zaznaczone — odznacz wyjątki przed generowaniem (OST zostaje). Zaznacz wszystko / Odznacz wszystko. Licznik Wybrane: N / M. Pojedynczy dokument — ikona pobierz przy szablonie generowanym. Nazwa ZIP np. GORLICKA_26_6_ODBIOR_WM.zip. Po udanym pobraniu wpis trafia do Historii (moduł WM Druk) i widać go też w szczegółach roboty."},
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              {q:"Komunikat o zapisie pipeline po notatce", a:"Zapis notatki idzie do chmury jako notatka operacyjna — nie zapisuje pipeline przetargów. Komunikat „Zapis pipeline zablokowany” / reconciliation dotyczy pracy z przetargami, nie samego „Zapisz” w Notatkach."},
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              {q:"Market Sync (P1)", a:"Tylko Super Admin: Biblioteka → Market Sync. Import CSV/JSON staging → Match → Accept (tylko staging) → link roboty (N:1) → Publish Summary → Confirm Publish. Zapis cen wyłącznie przez commitMarketQuotesImport. Kill Switch MARKET_SYNC_PUBLISH_ENABLED domyślnie OFF. Undo (jeden krok) po udanym Publish."},
              {q:"Market Sync Preview (P0)", a:"Warstwa Preview (import CSV + Match) pozostaje w tym samym panelu. Staging local-first (kw-market-sync-01-staging). Fuzzy OFF."},
              {q:"Czego tu nie ma?", a:"Market Sync nie zmienia AI-COST, Listy Płac ani Cloud Sync CORE. Origins Leroy/Casto nie wchodzą do średniej rynku, dopóki nie włączysz ich osobno w enabledOrigins. Brak auto-publish i scrapera."},
            ].map((item,i)=>(
              <div key={i} className="border border-border rounded-xl overflow-hidden">
                <div className="px-4 py-3 bg-secondary/30">
                  <p className="text-sm font-medium flex items-center gap-2"><HelpCircle size={13} className="text-primary shrink-0"/>{item.q}</p>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              {q:"Command Layer (detal przetargu)", a:"Sticky pasek u góry detalu workspace: Powrót · breadcrumb (Workspace › numer › tab; Decyzja › Kwalifikacja/Oferta) · tytuł · 5 zakładek. Następny krok (CTA workflow) na każdej zakładce w chrome. Pasek procesu (5 etapów) widoczny na wszystkich zakładkach — aktywny etap podświetlony („Tu jesteś”). Chip „Podsumowanie oferty” na każdej zakładce przenosi na Przetarg do sekcji intelligence hub (bez otwierania accordionu) — przewijanie odbywa się w obszarze treści pod chrome, nie w całym oknie. Chip skrótu wyceny (Kosztorys lub Wycena oferty — wg stanu kosztorysu) otwiera pierwszy tab kosztowy jednym klikiem z dowolnej zakładki; na mobile oba skróty są w jednym wierszu i mają min. 44 px wysokości. Na Przetargu dodatkowo: sygnały zaufania (zwijane) oraz wiersz „Następny krok wyceny” w hubie Podsumowanie oferty. Przy blockerach — chip „Blokery” w chrome. Na mobile (≤390 px) kompaktowe KPI pod zakładkami jest ukryte poza Przetargiem — KPI nadal widoczne na desktopie. Kosztorys — zwarty chrome. Menu Moduł (mobile) pokazuje aktywny workspace i powrót."},
              {q:"Most Kosztorys ↔ Ceny (workspace wyceny)", a:"Zakładki Kosztorys i Ceny są powiązane mostem nawigacji: pod paskiem fazy kosztorysu przycisk „Przejdź do wyceny oferty”; na wycenie — „Zobacz kosztorys ATH”. Pozycja scrollu na każdym z tych tabów jest zapamiętywana w tej sesji przeglądarki (po powrocie z drugiego tabu wracasz tam, gdzie skończyłeś czytać). Skrót w Command Layer i wiersz w hubie Przetargu prowadzą do sugerowanego pierwszego tabu kosztowego."},
              {q:"Podsumowanie oferty (intelligence hub)", a:"Na zakładce Przetarg, pod paskiem postępu V2, widzisz sekcję Podsumowanie oferty — skrót alertów workspace (do 3 pozycji) i kontekst przetargu. Nie zawiera werdyktu GO/HOLD — pełna rekomendacja jest na Decyzji. Z dowolnej zakładki (np. Dokumenty) jeden klik w chip „Podsumowanie oferty” w Command Layer otwiera Przetarg i przewija do tej sekcji."},
              ...
              {q:"Operator Action Bar", a:"Pasek akcji operacyjnych na Przetargu: e-Zamówienia · Wgraj SWZ · Uruchom ponownie analizę · Eksport PDF. Na desktopie (≥1024 px) pod Command Layer; na mobile i tablecie sticky na dole ekranu. Roboty i Usuń z listy pozostają w sekcji operatora (accordion Przygotowanie oferty)."},
              {q:"Pozycja w portfolio (Przetarg)", a:"Karta pod accordionem postępu: score, rekomendacja systemu, Twoja decyzja, ranking w portfolio i skrót powodów. Przycisk Strategia otwiera moduł Strategia z podświetleniem tego przetargu — po powrocie zostajesz w tym samym przetargu (bez osobnej zakładki strategia w detalu)."},
              ...
              {q:"Rozwiń przetarg — co się dzieje?", a:"Po wejściu w przetarg (dowolna zakładka V4) aplikacja w tle uruchamia pipeline: pobranie ogłoszenia i załączników BZP, ewentualnie pliki u zamawiającego, analizę dokumentów i wstępną wycenę — bez konieczności ręcznego startu. Zakładki: Przetarg (Workflow Hub) · Dokumenty · Kosztorys · Ceny · Decyzja. Przycisk „Uruchom ponownie analizę” wymusza ponowną analizę tylko gdy chcesz odświeżyć wyniki; „Szukaj u zamawiającego” gdy auto-discovery nie wystarczyło."},
              ...
              {q:"Zakładka Dokumenty — lista załączników", a:"W workspace Dokumenty załączniki są pogrupowane w siedem sekcji (accordion): SWZ, Przedmiary/ATH, Formularze ofertowe, Umowy, OPZ/STWiOR, Załączniki formalne i Pozostałe. Stan rozwinięcia grup jest zapamiętywany w tej sesji przeglądarki (po powrocie z Kosztorysu lub innej zakładki grupy pozostają tak jak je zostawiłeś). Źródło platformy (BZP/BIP) widać w badge przy nagłówku listy. Nad listą widać podsumowanie dokumentów (Summary Header). Nazwy plików są czytelniejsze (polskie znaki, spacje zamiast podkreśleń) — oryginalna nazwa pliku przy pobieraniu się nie zmienia."},
              {q:"Dokumenty — metadane analizy SWZ", a:"Pod listą załączników sekcja „Metadane analizy SWZ” jest domyślnie zwinięta — otwórz ją, aby zobaczyć datę ostatniej analizy, kryteria punktacji i fragmenty tabel z PDF. Status SWZ widać też w podsumowaniu dokumentów u góry zakładki."},
              {q:"Dokumenty — Odśwież BZP i Główna akcja", a:"Przyciski „Odśwież BZP” i „Szukaj u zamawiającego” są przy liście dokumentów (mniejsza, drugorzędna ścieżka). Tę samą akcję odświeżenia możesz też uruchomić Główną akcją u góry workspace, gdy CTA wskazuje pobranie lub analizę dokumentów."},
              ...
              {q:"Archiwum 7Z — komunikat o kosztorysie", a:"Gdy załącznik to plik .7z, aplikacja rozpakowuje go automatycznie (jak ZIP). „Błąd odczytu archiwum 7Z” oznacza problem techniczny z plikiem. „Zamawiający nie udostępnił kosztorysu ATH/XLS/XLSX w archiwum 7Z” oznacza, że archiwum odczytano poprawnie, ale nie było w nim kosztorysu inwestorskiego (często same PDF przedmiaru/projektów) — to nie jest błąd. Sprawdź przedmiar robót; wycenę możesz przygotować z ilości."},
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              {q:"Model wynagrodzenia — godzinowa czy akord?", a:"W edycji pracownika: Godzinowa (domyślnie) albo Akord. Godzinowa działa jak dotychczas. Akord to uzgodniona kwota, nie godziny. Zmiana modelu obowiązuje od następnego tygodnia na liście płac — bieżący tydzień i archiwum się nie zmieniają."},
              {q:"Akord na liście płac — obecność?", a:"Pracownik Akord jest na tej samej liście co godzinówka. W szczegółach dnia zaznaczasz tylko Był / Nie był (bez Od/Do). Obecność jest informacyjna — nie wylicza kwoty akordu ani godzin×stawka. Kwota uzgodniona i zaliczki AKORD: w kolejnych fazach."},
              {q:"Co to jest domyślna stawka?", a:"To stawka PLN za godzinę, którą ten pracownik zwykle zarabia. Będzie się automatycznie podpowiadać w Liście Płac i w Robotach. Możesz ją zmienić dla konkretnego tygodnia lub roboty — bez zmiany tej domyślnej. Przy modelu Akord stawka nie liczy wypłaty."},
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              {q:"Logowanie na iPhone (Safari) — spinner lub błąd", a:"Używaj oficjalnego adresu https://www.wgdom.fun. Jeśli przycisk «Zaloguj» długo się kręci lub pojawia się komunikat o przeglądarce albo pamięci — wyłącz «Zapamiętaj hasło», wyczyść dane strony w ustawieniach Safari albo zaloguj bez zapamiętywania. Samo logowanie nie wysyła hasła do chmury; problem dotyczy wyłącznie lokalnego zapisu w przeglądarce."},
