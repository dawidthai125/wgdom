/** WM-WORKER-SKETCH-01 P0 — Dokumentacja → Szkice (Worker mobile). */
...
import { filterWorkerSketchesForJob, getDrawingById } from "@/lib/wm-technical-drawings/merge";
...
export function WorkerJobSketchesSection({
  ...
  const mySketches = useMemo(
    () => filterWorkerSketchesForJob(drawings, jobId, workerId),
  ...
  const canEditSelected =
    selected &&
    selected.origin === "worker" &&
    selected.workflowStatus === "worker_draft" &&
    (!selected.createdByUserId || selected.createdByUserId === workerId);
    ...
    <WgCard elevation="soft" padding="sm" radius="lg" className="border-violet-500/25 space-y-3">
      ...
      {loading ? (
        ...
        <ul className="space-y-2">
          {mySketches.map((d) => {
            const editable = d.workflowStatus === "worker_draft";
            return (
              <li key={d.id}>
  ...
  if (!selected || typeof document === "undefined") return list;
    ...
    >
      ...
      {!canEditSelected && (
        <p className="text-[11px] text-muted-foreground text-center px-3 pb-2 shrink-0">
          Podgląd — szkic już przesłany (edycja po stronie weryfikacji w kolejnych etapach).
        </p>
      )}
    </div>,
    document.body,
