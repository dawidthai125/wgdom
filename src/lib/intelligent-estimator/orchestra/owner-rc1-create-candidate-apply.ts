/**
 * OWNER_RC1_CREATE_CANDIDATE — fail-closed apply helper for Safe Merge attestation.
 *
 * REUSE: resolveLaborIdentityMapping · applyAutonomousIdentityLeafToLine
 * Emits OWNER_RC1_CREATE_CANDIDATE + mappingId=<id>
 *   (≠ COMPOUND / CANONICAL_LEAF_REBIND / OWNER_RC1_VERIFY_CONNECT).
 * Does NOT hardcode MOPS lineIds · does NOT write storage.
 */
...
import {
  ...
  OWNER_RC1_CREATE_CANDIDATE_POLICY_VERSION,
  formatOwnerRc1CreateCandidateMappingIdToken,
  getOwnerRc1CreateCandidatePairForLeaf,
  getOwnerRc1CreateCandidatePairForMapping,
  isOwnerRc1CreateCandidateExactPair,
  isOwnerRc1CreateCandidateLeafWorkId,
} from "@/lib/work-catalog/owner-rc1-create-candidate-contract";

export const OWNER_RC1_CREATE_CANDIDATE_APPLY_VERSION =
  OWNER_RC1_CREATE_CANDIDATE_POLICY_VERSION;

export type OwnerRc1CreateCandidateApplyDecision =
  | "OWNER_RC1_CREATE_CANDIDATE_APPLY_ACCEPT"
  | "OWNER_RC1_CREATE_CANDIDATE_APPLY_REJECT";

export type OwnerRc1CreateCandidateApplyResult = {
  decision: OwnerRc1CreateCandidateApplyDecision;
  reasons: string[];
...
function reject(
  reasons: string[],
  partial?: Partial<OwnerRc1CreateCandidateApplyResult>,
): OwnerRc1CreateCandidateApplyResult {
  return {
    decision: "OWNER_RC1_CREATE_CANDIDATE_APPLY_REJECT",
    reasons,
 ...
 */
export function applyOwnerRc1CreateCandidateIdentityToLine(
  line: OfferBoqLine,
  input: {
    leafWorkId: string;
    mappingId: string;
  ...
  if (!isOwnerRc1CreateCandidateLeafWorkId(leaf)) {
    return reject(["LEAF_NOT_OWNER_RC1_CREATE_CANDIDATE"], {
      leafWorkId: leaf,
      mappingId,
    });
  }
  if (!isOwnerRc1CreateCandidateExactPair(leaf, mappingId)) {
    return reject(["LEAF_MAPPING_PAIR_MISMATCH"], { leafWorkId: leaf, mappingId });
  }

  const pairByLeaf = getOwnerRc1CreateCandidatePairForLeaf(leaf);
  const pairByMap = getOwnerRc1CreateCandidatePairForMapping(mappingId);
  if (!pairByLeaf || !pairByMap || pairByLeaf.mappingId !== pairByMap.mappingId) {
    return reject(["CREATE_PAIR_LOOKUP_FAIL"], { leafWorkId: leaf, mappingId });
  }
  ...
  if (!/CREATE_CANDIDATE/i.test(notes)) {
    return reject(["MAPPING_NOT_CREATE_CANDIDATE"], { leafWorkId: leaf, mappingId });
  }
  if (/VERIFY_CONNECT/i.test(notes)) {
    return reject(["MAPPING_IS_VERIFY_CONNECT_NOT_CREATE"], {
      leafWorkId: leaf,
      mappingId,
    });
  }
  ...
  const reasons = [
    OWNER_RC1_CREATE_CANDIDATE_ATTESTATION,
    formatOwnerRc1CreateCandidateMappingIdToken(mappingId),
    `policy=${OWNER_RC1_CREATE_CANDIDATE_POLICY_VERSION}`,
    "CREATE_CANDIDATE",
    "exact_normalized",
    "ownerApproval=true",
    "confidence=HIGH",
  ...
  if (!rationale.includes(OWNER_RC1_CREATE_CANDIDATE_ATTESTATION)) {
    return reject(["ATTESTATION_NOT_EMITTED"], { leafWorkId: leaf, mappingId });
  }
  if (!rationale.includes(formatOwnerRc1CreateCandidateMappingIdToken(mappingId))) {
  ...
  return {
    decision: "OWNER_RC1_CREATE_CANDIDATE_APPLY_ACCEPT",
