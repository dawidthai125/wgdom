/**
 * OWNER_RC1_VERIFY_CONNECT — fail-closed apply helper for Safe Merge attestation.
 *
 * REUSE: resolveLaborIdentityMapping · applyAutonomousIdentityLeafToLine
 * Emits OWNER_RC1_VERIFY_CONNECT + mappingId=<id> (≠ COMPOUND/CANONICAL_LEAF_REBIND invent).
...
import {
  ...
  OWNER_RC1_VERIFY_CONNECT_POLICY_VERSION,
  formatOwnerRc1VerifyConnectMappingIdToken,
  getOwnerRc1VerifyConnectPairForLeaf,
  getOwnerRc1VerifyConnectPairForMapping,
  isOwnerRc1VerifyConnectExactPair,
  isOwnerRc1VerifyConnectLeafWorkId,
} from "@/lib/work-catalog/owner-rc1-verify-connect-contract";

export const OWNER_RC1_VERIFY_CONNECT_APPLY_VERSION =
  OWNER_RC1_VERIFY_CONNECT_POLICY_VERSION;

export type OwnerRc1VerifyConnectApplyDecision =
  | "OWNER_RC1_VERIFY_CONNECT_APPLY_ACCEPT"
  | "OWNER_RC1_VERIFY_CONNECT_APPLY_REJECT";

export type OwnerRc1VerifyConnectApplyResult = {
  decision: OwnerRc1VerifyConnectApplyDecision;
  reasons: string[];
...
function reject(
  reasons: string[],
  partial?: Partial<OwnerRc1VerifyConnectApplyResult>,
): OwnerRc1VerifyConnectApplyResult {
  return {
    decision: "OWNER_RC1_VERIFY_CONNECT_APPLY_REJECT",
    reasons,
 ...
 */
export function applyOwnerRc1VerifyConnectIdentityToLine(
  line: OfferBoqLine,
  input: {
    leafWorkId: string;
    mappingId: string;
  ...
  if (!isOwnerRc1VerifyConnectLeafWorkId(leaf)) {
    return reject(["LEAF_NOT_OWNER_RC1_VERIFY_CONNECT"], { leafWorkId: leaf, mappingId });
  }
  if (!isOwnerRc1VerifyConnectExactPair(leaf, mappingId)) {
    return reject(["LEAF_MAPPING_PAIR_MISMATCH"], { leafWorkId: leaf, mappingId });
  }

  const pairByLeaf = getOwnerRc1VerifyConnectPairForLeaf(leaf);
  const pairByMap = getOwnerRc1VerifyConnectPairForMapping(mappingId);
  if (!pairByLeaf || !pairByMap || pairByLeaf.mappingId !== pairByMap.mappingId) {
    return reject(["CONNECT_PAIR_LOOKUP_FAIL"], { leafWorkId: leaf, mappingId });
  }
  ...
  if (!/VERIFY_CONNECT/i.test(notes)) {
    return reject(["MAPPING_NOT_VERIFY_CONNECT"], { leafWorkId: leaf, mappingId });
  }
  ...
  const reasons = [
    OWNER_RC1_VERIFY_CONNECT_ATTESTATION,
    formatOwnerRc1VerifyConnectMappingIdToken(mappingId),
    `policy=${OWNER_RC1_VERIFY_CONNECT_POLICY_VERSION}`,
    "VERIFY_CONNECT",
    "exact_normalized",
    "ownerApproval=true",
    "confidence=HIGH",
  ...
  // Hard: attestation tokens must be present for Safe Merge
  const rationale = String(patched.aiRationale || "");
  if (!rationale.includes(OWNER_RC1_VERIFY_CONNECT_ATTESTATION)) {
    return reject(["ATTESTATION_NOT_EMITTED"], { leafWorkId: leaf, mappingId });
  }
  if (!rationale.includes(formatOwnerRc1VerifyConnectMappingIdToken(mappingId))) {
    return reject(["MAPPING_ID_TOKEN_NOT_EMITTED"], { leafWorkId: leaf, mappingId });
  }

  return {
    decision: "OWNER_RC1_VERIFY_CONNECT_APPLY_ACCEPT",
